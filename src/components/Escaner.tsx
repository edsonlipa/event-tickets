...
function marcarLocalUsado(token: string, ingresoAt?: string | null) { const lista = leerLocal<Precargada[]>(CLAVE_LISTA, []); localStorage.setItem(CLAVE_LISTA, JSON.stringify(lista.map((item) => item.token === token ? { ...item, usado: true, ingresoAt: ingresoAt ?? item.ingresoAt } : item))); }
