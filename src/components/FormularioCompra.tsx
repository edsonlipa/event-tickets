...
export function FormularioCompra({ evento, correoSoporte }: { evento: Evento; correoSoporte: string }) {
  ...
  // Un dominio mal tecleado no falla al enviar: se detecta antes de cobrar.
  const correoSugerido = sugerirCorreo(datos.email);
  ...
  return <main data-compra-pasos className="event-compra-shell min-h-dvh bg-cream px-3 pt-5 text-ink sm:px-6 sm:pt-10"><div className="mx-auto max-w-md"><div className="relative z-10 mb-4 overflow-hidden shadow-brutal-sm"><NextImage src="/evento-banner.png" alt="II Open Championship" width={719} height={344} priority className="h-auto w-full" /></div>
    ...
    <div key={paso} className="motion-safe:animate-[step-in_.22s_ease-out]">{paso === 1 ?
      <form ref={formDatos} onSubmit={(e) => { e.preventDefault(); if (formDatos.current?.reportValidity()) { setError(""); setPaso(2); } }} className="event-panel space-y-5"><div><h2 className="text-2xl font-black uppercase">Entrada general</h2><p className="text-sm text-neutral-500">Completa tus datos antes de realizar el pago.</p><p className="mt-3 border-l-4 border-event-yellow bg-cream px-3 py-2 text-sm font-bold">Niños mayores de 5 años pagan entrada.</p></div>
        ...
        <Campo label="Correo"><input aria-label="Correo" required value={datos.email} onChange={(e) => setDatos({ ...datos, email: e.target.value })} type="email" autoComplete="email" maxLength={254} placeholder="maria@correo.com" className="ticket-input" /><p className="mt-2 border-l-4 border-event-yellow bg-cream px-3 py-2 text-xs font-bold text-ink">Revisa que tu correo esté bien escrito: ahí te enviaremos tus entradas.</p></Campo>
      ...
      <form onSubmit={enviar} className="overflow-hidden bg-white shadow-[0_0_0_1px_rgba(28,28,28,.12)]"><section className="border-b-2 border-dashed border-ink/20 p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black uppercase">Revisa tu compra</h2><p className="text-sm text-neutral-500">Confirma tus datos antes de pagar.</p></div><button type="button" onClick={() => setPaso(1)} className="text-xs font-black text-event-blue underline decoration-2 underline-offset-4">Editar datos</button></div><dl className="mt-5 grid gap-3 text-sm"><div><dt className="event-label">Comprador</dt><dd className="font-bold">{datos.nombre}</dd></div><div className="grid grid-cols-2 gap-3"><div><dt className="event-label">Celular</dt><dd className="font-bold">{datos.celular}</dd></div><div><dt className="event-label">Entradas</dt><dd className="font-bold">{datos.cantidad}</dd></div></div><div><dt className="event-label">Correo</dt><dd className="break-all font-bold">{datos.email}</dd>{correoSugerido && <div role="alert" className="mt-2 border-l-4 border-event-red bg-cream px-3 py-2 text-sm"><strong className="block font-black">¿Quisiste decir {correoSugerido}?</strong><span className="mt-1 block text-neutral-700">Si el correo está mal, tus entradas no te llegarán.</span><span className="mt-2 flex gap-4"><button type="button" onClick={() => { setDatos({ ...datos, email: correoSugerido }); }} className="font-black text-event-blue underline decoration-2 underline-offset-4">Usar el corregido</button><button type="button" onClick={() => setPaso(1)} className="font-black text-neutral-600 underline decoration-2 underline-offset-4">Escribirlo yo</button></span></div>}</div>{datos.nombres.some(Boolean) && <div><dt className="event-label">Nombres asignados</dt><dd className="font-bold">{datos.nombres.filter(Boolean).join(", ")}</dd></div>}</dl></section>
    ...
    </div><p className="mt-6 text-center text-sm text-neutral-600">¿Tienes problemas con el registro? Escríbenos a <a href={`mailto:${correoSoporte}`} className="font-bold text-event-blue underline decoration-2 underline-offset-4">{correoSoporte}</a></p><a href="/reenviar" className="mt-3 block text-center text-sm font-semibold text-event-blue underline decoration-2 underline-offset-4">¿No llegó tu correo? Reenviar entradas</a></div><PieDePagina /></main>;
}
