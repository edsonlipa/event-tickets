...
import { formatearFechaCsv } from "@/lib/fecha";
...
export async function GET() {
  ...
  const encabezado = ["id", "nombre_pagador", "celular", "email", "cantidad_personas", "precio_unitario", "monto_esperado", "status", "email_enviado", "entradas_activas", "entradas_usadas", "entradas_anuladas", "created_at", "created_at_peru"];
  const filas = (data ?? []).map((registro) => {
    const entradas = registro.entradas ?? [];
    return [registro.id, registro.nombre_pagador, registro.celular, registro.email, registro.cantidad_personas, registro.precio_unitario, registro.monto_esperado, registro.status, registro.email_enviado, entradas.filter((item) => !item.anulada).length, entradas.filter((item) => item.usado).length, entradas.filter((item) => item.anulada).length, registro.created_at, formatearFechaCsv(registro.created_at)];
  });
