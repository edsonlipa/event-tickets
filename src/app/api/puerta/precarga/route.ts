...
export async function GET() {
  ...
  const { data, error } = await getDb().from("entradas").select("id,nombre_persona,usado,usado_at,registros(nombre_pagador)").eq("anulada", false);
  ...
  return NextResponse.json({ entradas: (data ?? []).map((entrada) => {
    ...
    return { token: entrada.id, nombrePersona: entrada.nombre_persona, nombreComprador: Array.isArray(relacion) ? relacion[0]?.nombre_pagador : relacion?.nombre_pagador, usado: entrada.usado, ingresoAt: entrada.usado_at };
  }) });
}
