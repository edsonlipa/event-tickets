...
export default async function EntradaPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (!entrada || !evento) return <main className="event-shell grid place-items-center text-center"><section className="event-panel w-full max-w-md border-t-8 border-event-red"><p className="event-kicker">Código inválido</p><h1 className="event-title mt-2">Entrada no encontrada</h1><p className="mt-4 text-neutral-600">Verifica que el enlace esté completo.</p></section><PieDePagina /></main>;
