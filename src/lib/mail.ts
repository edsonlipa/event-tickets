...
async function armarAcuseCompra(registroId: string) {
  ...
  return {
    ...
    html: `<div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;color:#202020"><div style="background:#202020;color:#f3efe4;padding:28px"><p style="margin:0;color:#efc845;font-size:12px;font-weight:bold;letter-spacing:2px">COMPRA REGISTRADA</p><h1 style="margin:10px 0 0">Recibimos tu comprobante</h1></div><div style="padding:28px;border:1px solid #ddd"><p>Hola ${escapar(registro.nombre_pagador)}, registramos tu compra para <strong>${escapar(evento.nombre)}</strong>.</p><table style="width:100%;border-collapse:collapse;margin:24px 0"><tr><td style="padding:10px;background:#f3efe4">Entradas</td><td style="padding:10px;background:#f3efe4;text-align:right"><strong>${registro.cantidad_personas}</strong></td></tr><tr><td style="padding:10px">Precio unitario</td><td style="padding:10px;text-align:right"><strong>S/ ${Number(registro.precio_unitario).toFixed(2)}</strong></td></tr><tr><td style="padding:10px;background:#f3efe4">Total registrado</td><td style="padding:10px;background:#f3efe4;text-align:right"><strong>S/ ${Number(registro.monto_esperado).toFixed(2)}</strong></td></tr><tr><td style="padding:10px">Fecha</td><td style="padding:10px;text-align:right"><strong>${escapar(formatearFecha(evento.fecha))}</strong></td></tr>${evento.lugar ? `<tr><td style="padding:10px;background:#f3efe4">Lugar</td><td style="padding:10px;background:#f3efe4;text-align:right"><strong>${escapar(evento.lugar)}</strong></td></tr>` : ""}</table><div style="border-left:6px solid #efc845;background:#202020;color:#f3efe4;padding:16px"><strong>Tu pago está pendiente de verificación.</strong><br>Cuando el administrador lo confirme, recibirás un segundo correo con tus entradas y códigos QR.</div><p style="margin-top:24px;font-size:12px;color:#666">Código de registro: ${escapar(registro.id)}</p><p>¿Necesitas ayuda? Escríbenos a <a href="mailto:${escapar(remitente.replyTo)}">${escapar(remitente.replyTo)}</a>.</p></div></div>`,
    attachments: [],
